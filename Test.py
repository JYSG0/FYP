...
odrv0_errors = {"NONE": 0, "CONTROL_ITERATION_MISSED": 1, "DC_BUS_UNDER_VOLTAGE": 2, "DC_BUS_OVER_VOLTAGE": 4, "DC_BUS_OVER_REGEN_CURRENT": 8, "DC_BUS_OVER_CURRENT": 16, "BRAKE_DEADTIME_VIOLATION": 32, "BRAKE_DUTY_CYCLE_NAN": 64, "INVALID_BRAKE_RESISTANCE": 128}
...
def onupdate_errors():
    for error in error_objects:
        display = error[0]
        dict = error[1]
        if dict == odrv0_errors:
            try:
                update = getkey(odrv0.error, odrv0_errors)
            except:
                update = odrv0.error
        elif dict == axis_errors:
            try:
                update = getkey(odrv0.axis0.error, axis_errors)
            except:
                update = odrv0.axis0.error
        elif dict == motor_errors:
            try:
                update = getkey(odrv0.axis0.motor.error, motor_errors)
            except:
                update = odrv0.axis0.motor.error
        elif dict == encoder_errors:
            try:
                update = getkey(odrv0.axis0.encoder.error, encoder_errors)
            except:
                update = odrv0.axis0.encoder.error
        elif dict == controller_errors:
            try:
                update = getkey(odrv0.axis0.controller.error, controller_errors)
            except:
                update = odrv0.axis0.controller.error
...
def textbox_setting(container, label_text, path, info):
    ...
    display = displayobject(container, currentrow, 3, round(eval(f'odrv0.{path}'), 5))
...
def dropdown_setting(container, label_text, path, info, dict):
    ...
    display = displayobject(container, currentrow, 3, getkey(eval(f'odrv0.{path}'), dict))
...
def erase_configuration(container, column, columnspan):
    ...
    erase_configuration_button["command"] = lambda: eval("odrv0.erase_configuration()")
...
def clear_errors(container, column, columnspan):
    ...
    clear_errorsbutton["command"] = lambda: eval("odrv0.clear_errors()")
...
odrv0label = labelobject(frame, "Searching for an ODrive", "", 0, 0)
...
def show_after_started():
    ...
    error_display(section, "odrv0 error: ", odrv0.error, odrv0_errors)
    error_display(section, "Axis error: ", odrv0.axis0.error, axis_errors)
    error_display(section, "Motor error: ", odrv0.axis0.motor.error, motor_errors)
    error_display(section, "Encoder error: ", odrv0.axis0.encoder.error, encoder_errors)
    error_display(section, "Controller error: ", odrv0.axis0.controller.error, controller_errors)
    ...
    settings = [
        ...
        {"label": "Save motor calibration", "path": "axis0.motor.config.pre_calibrated", "info": "If true, the motor calibration will be saved to the odrv0 so you don't need motor startup calibration. Before setting this to true, you must perform motor calibration."}
    ]
