...
class YOLOP:
    ...
    def get_center_of_bbox(self, box):
        """ Calculate the center point of the bounding box """
        x_center = int((box[0] + box[2]) / 2)
        y_center = int((box[1] + box[3]) / 2)
        return (x_center, y_center)
    
    def detect(self, run_detection=True):
        cfg = self.cfg
        opt = self.opt
        ...
        for i, (path, img, img_det, vid_cap,shapes) in tqdm(enumerate(dataset),total = len(dataset)):
            ...
            if len(det):
                ...
                for *xyxy,conf,cls in reversed(det):
                    ...
                    plot_one_box(xyxy, img_det , label=label_det_pred, color=colors1[int(cls)], line_thickness=2)
            ...
            if dataset.mode == 'images':
                ...
                vid_writer.write(img_det)
                        ...
                        if run_detection:
                            ...
                            if boxes is not None:
                                for box, cls in zip(boxes, clss):
                                    ...
                                    center_point = self.get_center_of_bbox(box)
                    ...
                    key = 0xFF  # 0xFF ensures compatibility with 64-bit systems
                    
                    if key == ord('q'):  # Check if 'q' key is pressed
                        print("Exiting...")
                        return  # Exit the loop and close the window
